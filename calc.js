...
// ============================================================
  ...
  function searchAreas(q) {
    ...
    return AREA_LIST.filter(a => {
      ...
      return (GUGUN[a.area] || '').split(' ').some(g => g && (g === bare || g === k || g.includes(bare) || k.includes(g)));
    }).map(a => a.area);
  }
  ...
  // ============================================================
  const CATEGORIES = [
    { name: '식품',       icon: '🍱', note: '상하지 않는 것만 보낼 수 있습니다' },
    { name: '의류·패션',   icon: '👕', note: '' },
    { name: '생활용품',    icon: '🧴', note: '' },
    { name: '전자·디지털', icon: '💻', note: '충격에 약해 완충 포장이 필요합니다' },
    { name: '도서·문구',   icon: '📚', note: '' },
    { name: '화장품·미용', icon: '💄', note: '' },
    { name: '가구·침구',   icon: '🛏', note: '부피가 커서 요금이 올라갈 수 있습니다' },
    { name: '반려동물',    icon: '🐾', note: '살아있는 동물은 보낼 수 없습니다' },
  ];
